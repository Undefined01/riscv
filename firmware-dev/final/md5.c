...
const uint32_t r[64] = {7,  12, 17, 22, 7,  12, 17, 22, 7,  12, 17, 22, 7,
...
void to_bytes(uint32_t val, uint32_t *bytes) {
    bytes[0] = val & 0xff;
    bytes[1] = (val >> 8) & 0xff;
    bytes[2] = (val >> 16) & 0xff;
    bytes[3] = (val >> 24) & 0xff;
...
uint32_t to_int32(const uint32_t *bytes) {
    return bytes[0] | (bytes[1] << 8) | (bytes[2] << 16) | (bytes[3] << 24);
}

void md5(const uint32_t *initial_msg, uint32_t initial_len, uint32_t *digest) {
    // These vars will contain the hash
    uint32_t h0, h1, h2, h3;

    // Message (to prepare)
    static uint32_t msg[512 / 8];
    ...
    for (offset = 0; offset < initial_len; offset++)
        msg[offset] = initial_msg[offset];
    msg[initial_len] =
...
static void print8bithex(uint32_t hex) {
    int digit = (int)hex >> 4;
    rt_printchar(digit > 9 ? digit - 10 + 'a' : digit + '0');
    digit = hex & 0xf;
    rt_printchar(digit > 9 ? digit - 10 + 'a' : digit + '0');
}
...
int cmd_md5(int *args) {
    ...
    rt_printdec(len);
    rt_printstr(" : \"");
    for (int *i = args; *i; i++) rt_printchar(*i);
    rt_printstr("\" - ");
